...
import { FACULTY_AVATARS } from '../../services/api';
import {
  Plus,
  Search,
  Mail,
...
const InstructorsList = () => {
  ...
  const handleOpenAddModal = () => {
    ...
    const defaultAvatar = FACULTY_AVATARS[(instructors?.length || 0) % FACULTY_AVATARS.length];
    setInstructorForm({
      name: '',
      email: '',
  ...
  const getAssignedCourses = (assignedCourseIds = [], instName = '') => {
    return courses.filter(
      (c) =>
        assignedCourseIds.includes(c.id) ||
        (instName && c.instructor?.toLowerCase() === instName.toLowerCase())
    );
  };

  // Helper to get total students enrolled in instructor's courses
  const getStudentsTaughtCount = (assignedCourseIds = [], instName = '') => {
    const assignedCoursesList = getAssignedCourses(assignedCourseIds, instName);
    const assignedCourseIdsList = assignedCoursesList.map((c) => c.id);
    return enrollments.filter((e) => assignedCourseIdsList.includes(e.courseId)).length;
  };

  // Helper to get total course value generated by instructor
  const getInstructorCourseValue = (assignedCourseIds = [], instName = '') => {
    const assignedCoursesList = getAssignedCourses(assignedCourseIds, instName);
  ...
  return (
    <div className="space-y-6 font-sans animate-in fade-in duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {paginatedInstructors.length > 0 ? (
            paginatedInstructors.map((inst) => {
              const assignedCoursesList = getAssignedCourses(inst.assignedCourseIds, inst.name);
              const studentsTaught = getStudentsTaughtCount(inst.assignedCourseIds, inst.name);

              return (
                <div
                  key={inst.id}
                  className="bg-white rounded-3xl border border-sky-100 p-6 shadow-xs hover:shadow-xl hover:border-sky-300 transition-all duration-300 flex flex-col justify-between group space-y-4 relative overflow-hidden cursor-pointer"
                >
                  {/* Clickable Card Body (Opens Profile Modal) */}
                  <div onClick={() => setViewingInstructor(inst)} className="space-y-4 flex-1">
                  ...
                  {/* Actions Footer */}
                  <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                    <button
                  ...
                  paginatedInstructors.map((inst) => {
                    ...
                    return (
                      <tr
                        ...
                        className="hover:bg-sky-50/40 transition-colors duration-150 cursor-pointer"
                      >
                        {/* Instructor Info */}
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            ...
                            />
                            <div>
                              <p className="font-bold text-slate-800 text-sm hover:text-sky-600 transition-colors">{inst.name}</p>
                        ...
                        <td className="py-4 px-6 text-right" onClick={(e) => e.stopPropagation()}>
                          <div className="flex items-center justify-end gap-1">
                            <button
                              onClick={() => handleOpenEnrollStudentsModal(inst)}
                              className="p-2 rounded-xl hover:bg-sky-100 text-sky-600 transition-colors cursor-pointer"
                              title="Select & Enroll Students"
                            >
                              <UserPlus className="w-4 h-4" />
                            </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-sky-100 space-y-6 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmitInstructor} className="space-y-4 text-xs">
              ...
              {/* Faculty Profile Photo Preset Selection */}
              <div className="space-y-2">
                <label className="block font-bold text-slate-700 uppercase tracking-wider text-[11px]">
                  Select Faculty Profile Photo *
                </label>
                <div className="grid grid-cols-5 gap-2.5">
                  {FACULTY_AVATARS.map((url, idx) => (
                    <button
                      key={idx}
                      type="button"
                      onClick={() => setInstructorForm({ ...instructorForm, avatar: url })}
                      className={`relative rounded-2xl overflow-hidden aspect-square border-2 transition-all cursor-pointer group ${
                        instructorForm.avatar === url
                          ? 'border-sky-500 ring-4 ring-sky-500/20 scale-105 shadow-md'
                          : 'border-slate-200 hover:border-sky-300 opacity-80 hover:opacity-100'
                      }`}
                    >
                      <img src={url} alt={`Avatar ${idx + 1}`} className="w-full h-full object-cover" />
                      {instructorForm.avatar === url && (
                        <div className="absolute inset-0 bg-sky-500/40 backdrop-blur-[1px] flex items-center justify-center">
                ...
                <p className="text-[10px] text-slate-500 font-medium">
                  Click a photo preset above, or specify a custom portrait URL below:
                </p>
                <input
                  type="url"
                  ...
                  className="w-full px-4 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 text-xs font-mono"
                />
              </div>
      ...
      {viewingInstructor && (() => {
        const assignedCoursesList = getAssignedCourses(viewingInstructor.assignedCourseIds, viewingInstructor.name);
        const assignedCourseIdsList = assignedCoursesList.map((c) => c.id);
        const instructorStudents = enrollments.filter((e) =>
          assignedCourseIdsList.includes(e.courseId)
        );
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sky-600 font-mono text-[11px] font-bold">
                      Total Value: ${getInstructorCourseValue(viewingInstructor.assignedCourseIds, viewingInstructor.name)}
                    </span>
                  </div>
